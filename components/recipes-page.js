/* ============================================================
   VBG — Recipes page controller
   - Builds the filter sidebar from window.VBG_RECIPES facets
   - Wires every control to <vbg-recipes-table>.setFilters(...)
   - Keeps result count / footer / active-filter chips in sync
   - Opens the full recipe detail page on row click / "Open"
   ============================================================ */
(function () {
  function ready(fn) {
  ...
  ready(function () {
    ...
    /* ---------------- filter state ---------------- */
    var state = {
      owner: new Set(), category: new Set(), status: new Set(),
      major: new Set(), tags: new Set(),
    ...
    var SECTIONS = [
      {
        id: 'owner', title: 'Ownership', type: 'check', key: 'owner',
        opts: [
          { val: 'vbg',     label: 'VBG',     n: count(function (r) { return r.owner === 'vbg'; }) },
          { val: 'company', label: 'Company', n: count(function (r) { return r.owner === 'company'; }) }
        ]
      },
      {
        id: 'category', title: 'Category', type: 'check', key: 'category', dot: true,
        opts: Object.keys(CATS).map(function (k) {
          return { val: k, label: CATS[k].label, n: count(function (r) { return r.category === k; }) };
        })
      },
      {
        id: 'status', title: 'Status', type: 'check', key: 'status',
        opts: [
          { val: 'published', label: 'Published', n: count(function (r) { return r.status === 'published'; }) },
          { val: 'draft',     label: 'Draft',     n: count(function (r) { return r.status === 'draft'; }) },
          { val: 'archived',  label: 'Archived',  n: count(function (r) { return r.status === 'archived'; }) }
        ]
      },
      {
        id: 'major', title: 'Version', type: 'check', key: 'major',
        opts: [3, 2, 1, 0].map(function (m) {
          return { val: String(m), label: m === 0 ? 'Pre-release (v0.x)' : ('v' + m + '.x'), n: count(function (r) { return r.major === m; }) };
        }).filter(function (o) { return o.n > 0; })
      },
      {
        id: 'tags', title: 'Tags', type: 'chips', key: 'tags',
        opts: R.allTags().map(function (t) { return { val: t.tag, label: t.tag, n: t.n }; })
      },
      {
      ...
      },
      {
        ...
        opts: [
          ...
          { val: 'low',    label: '1–5 projects' },
          { val: 'mid',    label: '6–10 projects' },
          { val: 'high',   label: '10+ projects' }
        ]
      }
    ];
    ...
    function resetAll() {
      state.owner.clear(); state.category.clear(); state.status.clear();
      state.major.clear(); state.tags.clear();
    ...
    /* create → open the create-recipe flow */
    document.getElementById('create-btn').addEventListener('click', function () {
      location.href = 'CreateRecipe.html';
    });
    ...
    function syncActiveChips() {
      ...
      state.major.forEach(function (v) { chips.push({ k: 'major', v: v, t: majorLabel(v) }); });
    ...
    table.addEventListener('recipesrender', function (e) {
      var d = e.detail;
      if (rcount) {
        rcount.innerHTML = '<b>' + d.visible + '</b> recipe' + (d.visible === 1 ? '' : 's')
          + (d.visible !== d.total ? ' <span style="color:var(--fg4)">of ' + d.total + '</span>' : '');
      }
      if (footnote) footnote.textContent = 'Showing ' + d.visible + ' of ' + d.total + ' recipes';
    });

    /* ======================================================
       OPEN RECIPE → navigate to the full detail page
       ====================================================== */
    table.addEventListener('recipeopen', function (e) {
      var r = e.detail.recipe;
      location.href = 'RecipeDetail.html?r=' + encodeURIComponent(r.code);
    });
  });
})();
